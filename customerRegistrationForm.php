...
if(isset($_POST["submit"]))
{
  ...
  if (!empty($_POST['inregister'])) {
  $register = $_POST['inregister'];
  } else {
  $flag = 0;
  }

  if (!empty($_POST['inbadge'])) {
  $badge = $_POST['inbadge'];
  } else {
    $flag = 0;
  }

  if (!empty($_POST['infriday'])) {
  $friday = $_POST['infriday']; 
  } 

  if (!empty($_POST['insaturday'])) {
  $saturday = $_POST['insaturday'];
  } 

  if (!empty($_POST['insunday'])) {
  $sunday = $_POST['insunday'];
  } 

  if (!empty($_POST['inother'])) {
    $other = $_POST['inother'];
  } 

  if (!empty($_POST['inname2'])) {
  $test = $_POST['inname2']; }

  if ($flag == 1 and !$test == 1) {
  ...
  echo "<p>Form Submitted!</p>";
    ...
    echo "<p>Form not complete, please try again.</p>";
  }
